...
let url='https://api.privatbank.ua/p24api/infrastructure?json&atm&address=&city=%D0%94%D0%BD%D1%96%D0%BF%D1%80%D0%BEs';
fetch(url)
...
.then(result=>{
  ...
  navigator.geolocation.getCurrentPosition(pos=>{
   ...
   .then(result=>{
    ...
    var new_adress;//=result.Response.View[0].Result[0].Location.Address.Label;
    if(result.Response.View[0].Result[0].MatchLevel=='houseNumber'){
      new_adress=result.Response.View[0].Result[0].Location.Address.Label;
    }else{
      new_adress=result.Response.View[0].Result[1].Location.Address.Label;
    }
...
ui.addBubble(bubble);
   ...
   var bubble = new H.ui.InfoBubble({ lng: `${new_atms.lon}`, lat: `${new_atms.lat}` }, {
    content: `${add_2}, відстань ${new_atms.distance} м`
  });
